 ...
 */
template <class T>
class ACCESSION_API TAccessionNodeFactory : public FGraphNodeFactory
{
public:
	static_assert(TIsDerivedFrom<T, FGraphNodeFactory>::IsDerived, "Provided Template Type Must Derive From FGraphNodeFactory");

	TAccessionNodeFactory()
	{
		Implementation = TSharedPtr<T>();

		AccessionRegistry = FAccessionModule::Get().AssetRegistry.ToSharedRef();
	}

	TAccessionNodeFactory(TSharedRef<FAccessionAssetRegistry> InAccessionRegistry)
		: AccessionRegistry(InAccessionRegistry)
	{
		Implementation = TSharedPtr<T>();
	}

	virtual ~TAccessionNodeFactory()
	{
	}
...
protected:
	/**
	 * The Asset Registry of the Accession Plugin.
	 */
	TSharedRef<FAssetAccessionRegistry> AccessionRegistry;

	TSharedPtr<T> Implementation;
};

template <class T>
TSharedPtr<class SGraphNode> TAccessionNodeFactory<T>::CreateNodeWidget(UEdGraphNode *InNode)
{
	check(InNode != nullptr);

	TSharedPtr<SGraphNode> OutNode = Implementation->CreateNodeWidget(InNode);

	// Apply Accession Visuals to the Node.

	TSharedRef<FGraphIndexer> GraphIndexer = AccessionRegistry->GetGraphIndexer(InNode->GetGraph());
...
template <class T>
TSharedPtr<class SGraphPin> TAccessionNodeFactory<T>::CreatePinWidget(UEdGraphPin *InPin)
{
	check(InPin != nullptr);
	...
	TSharedRef<FGraphIndexer> GraphIndexer = AccessionRegistry->GetGraphIndexer(InPin->GetOwningNode()->GetGraph());
	...
	{
		OutPin->SetContent(
			SNew(SHorizontalBox) + SHorizontalBox::Slot()[PinWidgetContent] + SHorizontalBox::Slot()[AccessiblityWidget]);
	...
	{
		OutPin->SetContent(
			SNew(SHorizontalBox) + SHorizontalBox::Slot().AutoWidth()[AccessiblityWidget] + SHorizontalBox::Slot().AutoWidth()[PinWidgetContent]);
