...
UGraphLocomotionContext::UGraphLocomotionContext(const FObjectInitializer &ObjectInitializer)
...
UGraphLocomotionContext::~UGraphLocomotionContext()
{
	...
	UE_LOG(LogAccession, Warning, TEXT("GraphLocomotion: CONTEXT DESTROYED."));
}

void UGraphLocomotionContext::Init()
{
	{
		TSharedPtr<SDockTab> ActiveTab = FGlobalTabmanager::Get()->GetActiveTab();
		if (!ActiveTab.IsValid())
		{
			UE_LOG(LogAccession, Warning, TEXT("GraphLocomotion: NO ACTIVE TAB FOUND."));
			return;
		}
		...
		{
			UE_LOG(LogAccession, Warning, TEXT("GraphLocomotion: CURRENT ACTIVE TAB IS NOT OF TYPE - SGraphEditor"));
			return;
		}
	}
...
void UGraphLocomotionContext::Init(const TSharedRef<SGraphEditor> InGraphEditor)
{
...
bool UGraphLocomotionContext::SelectChunk(const int32 &Index)
{
	...
	GEditor->GetTimerManager()->SetTimer(
		...
		[this, Index, GraphTopLeftCoord, GraphBottomRightCoord]()
		{
			...
			{
				UE_LOG(LogAccession, Log, TEXT("Failed To Jump To Viewport Coords (TopLeft: %s | BottomRight: %s)"),
...
bool UGraphLocomotionContext::RevertToPreviousView()
{
...
void UGraphLocomotionContext::ConfirmSelection()
{
	Close();
}

void UGraphLocomotionContext::CancelLocomotion()
{
	if (LinkedEditor.IsValid())
	{
...
bool UGraphLocomotionContext::Close()
{
	...
	UE_LOG(LogAccession, Warning, TEXT("GraphLocomotion: CONTEXT CLOSED."));

	return true;
}

bool UGraphLocomotionContext::MoveViewport(const FVector2D &InTopLeft, const FVector2D &InBottomRight) const
{
	if (!LinkedEditor.IsValid())
		return false;
...
bool UGraphLocomotionContext::MoveViewport(const FPanelViewPosition &NewViewPosition) const
{
	if (!LinkedEditor.IsValid())
		return false;
...
void UGraphLocomotionContext::ChangeChunkVis(const int32 &Index, const FLinearColor &NewColor)
{
	check(Index < ChunkArray.Num() && Index >= 0)

		ChunkArray[Index]
			.SetVisColor(NewColor);
}

void UGraphLocomotionContext::CreateVisualGrid(const TSharedRef<SGraphEditor> InGraphEditor)
{
	...
	{
		UE_LOG(LogAccession, Warning, TEXT("GraphLocomotion: NO GRAPH VIEWPORT FOUND."));
		return;
	}
...
void UGraphLocomotionContext::GenerateVisualChunks(const TSharedRef<SGraphEditor> InGraphEditor, FIntVector2 InVisualChunkSize)
{
...
void UGraphLocomotionContext::CalculateVisualChunksBounds()
{
	if (!LinkedEditor.IsValid())
		return;
...
void UGraphLocomotionContext::RemoveVisualGrid()
{
	TSharedPtr<SUniformGridPanel> GridContainerPtr = GridContainer.Pin();
	if (GridContainerPtr.IsValid())
	{
		...
		if (ParentWidget.IsValid())
		{
			...
			UE_LOG(LogAccession, Warning, TEXT("GraphLocomotion: PARENT WIDGET NOT FOUND, CANNOT REMOVE LOCOMOTION WIDGETS."))
	}
}

void UGraphLocomotionContext::HideNativeVisuals()
{
...
void UGraphLocomotionContext::OnFocusChanged(
	...
	const FWidgetPath &NewFocusedWidgetPath, const TSharedPtr<SWidget> &NewFocusedWidget)
{
	...
	UE_LOG(LogAccession, Warning, TEXT("GraphLocomotion: FOCUS CHANGED."));
...
void UGraphLocomotionContext::BindFocusChangedEvent()
{
	FocusChangedHandle = FSlateApplication::Get().OnFocusChanging().AddUObject(this, &UGraphLocomotionContext::OnFocusChanged);
}

void UGraphLocomotionContext::UnbindFocusChangedEvent()
{
