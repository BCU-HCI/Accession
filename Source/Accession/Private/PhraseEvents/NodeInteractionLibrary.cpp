																																		...
																																		TPhraseNodeArray{
																																			...
																																			MakeShared<FPhraseNode>(TEXT("CONNECT"),
																																									TPhraseNodeArray{
																																										...
																																										MakeShared<FPhraseContextMenuNode<UGraphEditorContext>>(
																																											TEXT("ADD"),
																																											1.5f,
...
void UNodeInteractionLibrary::MoveNode(FParseRecord &Record)
{
	...
	if (Node == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("MoveNode: Node Not Found"));
		return;
	}
	...
	default:
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("MoveNode: Invalid Direction"));
		break;
	}
}
...
void UNodeInteractionLibrary::DeleteNode(FParseRecord &Record)
{
	...
	if (Node == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("DeleteNode: Node Not Found"));
		return;
	}
...
void UNodeInteractionLibrary::RequestRename(FParseRecord &Record)
{
	...
	if (IndexInput == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("RequestRename: Invalid Node Index"));
		return;
	}
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("RequestRename: Node Not Found"));
		return;
	}
...
void UNodeInteractionLibrary::PinConnect(FParseRecord &Record)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinConnect: Invalid Inputs Amount"));
		return;
	}
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinConnect: Pins Not Found"));
		return;
	}

	if (!Graph->GetSchema()->TryCreateConnection(SourcePin, TargetPin))
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinConnect: Pin Connection Failed"));
	}
}
...
void UNodeInteractionLibrary::PinDisconnect(FParseRecord &Record)
{
	...
	if (NodeInputs.Num() != 2 || PinInputs.Num() != 2)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinDisconnect: Invalid Inputs Amount"));
		return;
	}
	...
	if (SourcePin == nullptr || TargetPin == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinDisconnect: Pins Not Found"));
		return;
	}
...
void UNodeInteractionLibrary::PinSetDefault(FParseRecord &Record)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinSetDefault: Invalid Node / Pin Inputs"));
		return;
	}

	if (DefaultValueInput == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinSetDefault: Invalid Value Input"));
		return;
	}
	...
	if (FoundPin == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinSetDefault: Pin Not Found"));
		return;
	}

	if (FoundPin->bDefaultValueIsReadOnly || FoundPin->bDefaultValueIsIgnored)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinSetDefault: Pin Default Value Is InAccessible"));
		return;
	}
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PinSetDefault: Provided Value Is Invalid - %s"), *DefaultSetLog);
		return;
	}
...
TSharedPtr<IMenu> UNodeInteractionLibrary::NodeAddMenu(FParseRecord &Record)
{
	...
	FVector2D SpawnLocation;
	{
		...
		{
			UE_LOG(LogAccessionPhraseEvent, Display, TEXT("NodeAddMenu: IMenu Could Not Be Found In Widget Path"))
			return TSharedPtr<IMenu>();
		}
	}
}
...
TSharedPtr<IMenu> UNodeInteractionLibrary::NodeAddPinMenu(FParseRecord &Record)
{
	...
	FVector2D SpawnLocation;
	{
		...
		{
			UE_LOG(LogAccessionPhraseEvent, Display, TEXT("NodeAddMenu: Invalid Inputs"));
...
{
	GET_TOP_CONTEXT(Record, ContextMenu, UGraphEditorContext)
...
void UNodeInteractionLibrary::NodeAddScroll(FParseRecord &Record)
{
	...
	default:
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("NodeAddScroll: Invalid Scroll Position / Direction"));
		return;
	}
}
...
void UNodeInteractionLibrary::SelectionNodeToggle(FParseRecord &Record)
{
	...
	if (Node == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Warning, TEXT("SelectionToggle: Node Not Found"));
		return;
	}
...
void UNodeInteractionLibrary::SelectionMove(FParseRecord &Record)
{
	...
	for (UObject *NodeObj : ActiveGraphEditor->GetSelectedNodes())
	{
		...
		default:
			UE_LOG(LogAccessionPhraseEvent, Display, TEXT("SelectionMove: Invalid Direction"));
			return;
		}
	}
}
...
void UNodeInteractionLibrary::SelectionComment(FParseRecord &Record)
{
	...
	if (CommentCreateAction.IsValid())
	{
		...
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("SelectionComment: Comment Creation Failed"));
}
...
void UNodeInteractionLibrary::LocomotionSelect(FParseRecord &Record)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Warning, TEXT("Locomotion Select: Failed to Choose New View."));
	}
}
...
void UNodeInteractionLibrary::LocomotionRevert(FParseRecord &Record)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Warning, TEXT("Locomotion Revert: Failed to Revert to Previous View."));
	}
}
...
void UNodeInteractionLibrary::BlueprintCompile(FParseRecord &Record)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("BlueprintCompile: Active Graph Not Found"));
		return;
	}
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("BlueprintCompile: Blueprint Not Found"));
		return;
	}
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Warning, TEXT("BlueprintCompile: BlueprintEditor Not Found"));
		return;
	}
...
UNodeInteractionLibrary::GridAttributes UNodeInteractionLibrary::GetGridAttributes(const SGraphPanel *Panel)
{
	if (Panel == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("GetGridAttributes: Invalid Panel"));
	...
	UE_LOG(LogAccessionPhraseEvent, Display, TEXT("GridAttributes: GridCellSize: %f, NominalGridSize: %f, InflationFactor: %f, ZoomFactor: %f"), GridCellSize, NominalGridSize, InflationFactor, ZoomFactor);
...
void UNodeInteractionLibrary::SnapToGrid(const SGraphPanel *Panel, UEdGraphNode *Node)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("SnapToGrid: Invalid Panel / Node"));
		return;
	}
...
void UNodeInteractionLibrary::MoveOnGrid(const SGraphPanel *Panel, UEdGraphNode *Node, const FVector2D &MovementDelta)
{
	if (Panel == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("MoveOnGrid: Invalid Panel"));
		return;
	}
...
void UNodeInteractionLibrary::SnapToGridCentre(const SGraphPanel *Panel, UEdGraphNode *Node)
{
	if (Panel == nullptr || Node == nullptr)
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("CenterToGrid: Invalid Panel / Node"));
		return;
	}
...
FVector2D GraphCoordToPanelCoord(FVector2D PanelCoord, const SGraphPanel *GraphPanel)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("PanelCoordToGraphCoord: Invalid Graph Panel"));
...
FVector2D UNodeInteractionLibrary::GetFreeGraphViewportSpace(const SGraphEditor *GraphEditor)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("GetFreeGraphViewportSpace: Invalid Graph Editor"));
		return FVector2D::ZeroVector;
	}
	...
	FVector2D PanelSize = GraphPanel->GetCachedGeometry().GetLocalSize();
	{
		...
		UE_LOG(LogAccession, Log, TEXT("Calculated Node-Based Density Grid Resolution: %s"), *GridResolution.ToString());
...
int32 UNodeInteractionLibrary::GetNodesInViewport(const SGraphEditor *GraphEditor, const SGraphPanel *GraphPanel, TArray<UEdGraphNode *> &GraphNodes)
{
	...
	{
		UE_LOG(LogAccessionPhraseEvent, Display, TEXT("GetNodesInViewport: Invalid Graph Widget is nullptr"));
		return INDEX_NONE;
	}
